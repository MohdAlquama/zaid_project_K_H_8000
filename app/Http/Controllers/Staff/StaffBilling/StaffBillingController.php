...
namespace App\Http\Controllers\Staff\StaffBilling;
...
class StaffBillingController extends Controller
{
    ...
    public function view(Request $request): Response
    {
        ...
        $billings->setCollection(
            $billings->getCollection()->map(fn (Billing $billing): array => [
                ...
                'can_edit' => $this->canEditBilling($billing),
                'can_delete' => $this->canDeleteBilling($billing),
            ])->values()
        );

        return Inertia::render('staff/Billing/ViewBilling', [
            'billings' => $billings,
            'filters' => $filters,
        ]);
    }
  ...
  $settings = InvoiceControl::first() ?? new InvoiceControl();
    ...
    public function update(Request $request, Billing $billing): RedirectResponse
    {
        ...
        if (! $request->exists('frames') && ! $request->exists('lenses')) {
            ...
            $billing->update([
                ...
                'mobile_number' => trim((string) ($validated['mobile_number'] ?? '')) ?: 'n/a',
                'delivery_date' => $validated['delivery_date'] ?? null,
            ]);

            return redirect()
                ->route('staff.billing.update-page', $billing->fresh())
                ->with(
                    'success',
                    ($forceUpdate ? 'Billing force updated successfully' : 'Billing updated successfully')
        ...
        return redirect()
            ->route('staff.billing.update-page', $billing)
            ->with(
                'success',
                ($forceUpdate ? 'Billing force updated successfully' : 'Billing updated successfully')
                    .". Order number: {$billing->order_number}"
    ...
    {
        $this->ensureDeleteWindow($billing);
    ...
    private function canEditBilling(Billing $billing): bool
    {
        return $this->isWithinWindow($billing, 10);
    }

    private function canDeleteBilling(Billing $billing): bool
    {
        return $this->isWithinWindow($billing, 6);
    }

    private function ensureEditWindow(Billing $billing): void
    {
        abort_unless(
            $this->canEditBilling($billing),
            403,
            'Billing can only be edited within 10 minutes of creation.'
        );
    }

    private function ensureDeleteWindow(Billing $billing): void
    {
        abort_unless(
            $this->canDeleteBilling($billing),
            403,
            'Billing can only be deleted within 6 minutes of creation.'
        );
    }
