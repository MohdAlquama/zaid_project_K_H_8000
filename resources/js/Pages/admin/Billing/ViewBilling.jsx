...
function ViewBilling({ billings = emptyBillings, filters = emptyFilters }) {
  ...
  const applyFilters = (event) => {
    ...
    router.get(route('admin.billing.view'), payload, {
      preserveState: true,
      preserveScroll: true,
      replace: true,
    })
  ...
  const resetFilters = () => {
    ...
    router.get(route('admin.billing.view'), {}, {
      preserveState: true,
      preserveScroll: true,
      replace: true,
    })
  ...
  const submitPaymentCollection = (event) => {
    ...
    paymentForm.post(route('admin.billing.collect-payment', selectedBilling.id), {
      preserveScroll: true,
      onSuccess: () => {
        closePaymentDialog()
      },
  ...
  const deleteBilling = (billing) => {
    ...
    router.delete(route('admin.billing.destroy', billing.id), {
      preserveScroll: true,
    })
  }
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">
              Search billing records by customer name, order number, mobile number, order date, or delivery date.
            </p>
          </div>

          <Button asChild>
            <Link href={route('admin.create-billing')}>Create Billing</Link>
          </Button>
        </div>
      </div>
                      ...
                      <TableCell>
                        <div className="flex justify-end gap-2">
                          <Button asChild size="sm" variant="outline">
                            <Link href={route('admin.billing.edit', billing.id)}>View</Link>
                          </Button>
                          <Button asChild size="sm" variant="outline">
                            <Link href={route('admin.billing.invoice', billing.id)}>Invoice</Link>
                          </Button>
                          <Button
                            type="button"
                            size="sm"
          ...
          <div className="flex flex-col gap-4 border-t px-6 py-4 md:flex-row md:items-center md:justify-between">
            <p className="text-sm text-slate-600">
              {totalBillings === 0
                ...
                : `Showing ${fromRecord} to ${toRecord} of ${totalBillings} billing records.`}
            </p>
...
ViewBilling.layout = page => <AdminLayout children={page} />
