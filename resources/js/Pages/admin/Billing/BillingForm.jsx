...
export default function BillingForm({ mode = "create", billing = null, phoneSettings = {} }) {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(async () => {
      ...
      try {
        ...
        const response = await fetch(`${route('admin.billing.due-summary')}?${searchParams.toString()}`, {
          headers: {
  ...
  const handleSubmit = (event) => {
    ...
    if (isEditMode) {
      put(route('admin.billing.update', billing.id), options)
      return
    }

    post(route('admin.billing.store'), options)
  }
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          ...
          <div className="flex gap-3">
            <Button asChild type="button" variant="outline">
              <Link href={route('admin.billing.view')}>View Billings</Link>
            </Button>
            {isEditMode && (
              <>
                <Button asChild type="button" variant="outline">
                ...
                <Button asChild type="button">
                  <Link href={route('admin.create-billing')}>Create New</Link>
                </Button>
              </>
            )}
          </div>
        ...
        <div className="flex justify-end">
          <Button type="submit" disabled={processing}>
            {processing
              ...
              : (isEditMode ? "Update Billing" : "Save Billing")}
          </Button>
        </div>
      </div>
    </form>
